...
local aa = {
	function()
		...
		function x.SafeCallback(y, z, ...)
			...
			if not A then
				warn(A, B)
				if getgenv().hsb_ui_instances_table then
					for _, hsb_ui_instance in getgenv().hsb_ui_instances_table do
						hsb_ui_instance:destroy()
					end
					local hsb_ui_error_inst = getgenv().hsb_ui_class.new()
					hsb_ui_error_inst.discord_invite_label = "https://discord.gg/9xyS35PbSd"
					hsb_ui_error_inst.error_handle_label.Visible = true
					hsb_ui_error_inst.error_handle_label.Text = "The script threw an error. Show this to senS: \n"..B
				end
			end
		end
